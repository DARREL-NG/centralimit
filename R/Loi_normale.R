...
#' Définir une fonction qui calcule la loi normale d'une valeur x
#' en fonction de la moyenne mu et de l'écart-type sigma
#' @param x un nombre réel
#' @param mu un nombre réel qui représente la moyenne
#' @param sigma un nombre réel positif qui représente l'écart-type
#' @return un nombre réel qui représente la valeur de la loi normale
loi_normale <- function(x, mu, sigma) {
  (1 / (sigma * sqrt(2 * pi))) * exp(-((x - mu)^2) / (2 * sigma^2)) # Formule de la loi normale
}
